...
Lines will consist of two points and be connected using xialoin wus line drawing algorithm.
...
Currently the CMakeLists.txt assumes that all required packages will be available and downloaded using a package manager. To make the program more general, the CMakeLists.txt can check whether the packages were found and if not, download them from github.

The general CMakeLists.txt code for this is:
```cmake
find_package([PACKAGE_NAME] CONFIG COMPONENTS [components])
if(NOT [PACKAGE]_FOUND)
    message("[PACKAGE] NOT FOUND")
    FetchContent_Declare( [PACKAGE_NAME] 
        GIT_REPOSITORY [PACKAGE GIHUB LINK]
...
This works for most of the packages however, ImGui-SFML would fail trying to use find_package(SFML) within its own CMakeLists.txt. Since the package is not downloaded using a package manager, this fails and the build crashes.

![CMakeListsIMGUISFMLFindPackageError](imgs/CMakeListsImGuiSFMLFindPackageError.png)

To fix this I looked into what the CMakeLists was doing:

#### _imgui-src/CMakeLists.txt_
```cmake
...
This part of the CMakeLists.txt is where Imgui-SFML tries to find SFML
To stop this from happening the variable IMGUI_SFML_FIND_SFML needs to be changed to OFF
